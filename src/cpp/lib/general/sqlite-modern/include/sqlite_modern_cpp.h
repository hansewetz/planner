	...
	void _extract(std::function<void(void)> call_back) {
		...
		if (hresult != SQLITE_DONE) {
			throw_sqlite_error(hresult);
               ...
               /* ewetzha: commented out since the statement will never be destroyed by 
                           sqlite3_finalize in sqlite3_statment dtor
		_stmt = nullptr;
               */
	}
...
/* ewetzha: specialisation moved to library to avoid errors witrh ODR (One Definition Rule)
// int
